...
internal class ElvishSetLValueReference(element: ElvishSetLValue, rangeInElement: TextRange?) :
    PsiReferenceBase<ElvishSetLValue?>(element, rangeInElement, true),
    PsiPolyVariantReference {
    private val log = logger<ElvishSetLValueReference>()
    ...
    internal open class VariableFinder(private val target: ElvishVariableName) : ElvishBlockClimber() {
        ...
        private fun ElvishChunk.matchingVariables(): Pair<Collection<ElvishLValueVariable>, Boolean> {
            val variables = variableDeclarations.filter { it.matches() }
